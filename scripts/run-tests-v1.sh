...
APP="$DERIVED_DATA_PATH/Build/Products/Debug/Kolt DEV.app"
RUN_TAG="tests-v1"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: Kolt DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "Kolt DEV" || true
  pkill -x "cmux" || true
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "Kolt DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done
  ...
  CMUX_TAG="$RUN_TAG" CMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/Kolt DEV" >/dev/null 2>&1 &
