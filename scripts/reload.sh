...
print_tag_cleanup_reminder() {
  ...
  if [[ "${#stale_tags[@]}" -eq 0 ]]; then
    ...
    for tag in "${stale_tags[@]}"; do
      echo "  pkill -f \"Kolt DEV ${tag}.app/Contents/MacOS/Kolt DEV\""
  ...
  echo "  pkill -f \"Kolt DEV ${current_slug}.app/Contents/MacOS/Kolt DEV\""
